"""JSON and Markdown reports for AP-E7-007."""
...
class RecoveryAcceptanceReport(dict[str, Any]):
    @classmethod
    def new(
...
def write_report(
    report: RecoveryAcceptanceReport, output: str | Path
) -> tuple[Path, Path]:
...
def _markdown(report: RecoveryAcceptanceReport) -> str:
    lines = [
        f"# AP-E7-007 Recovery Report: `{report['plan_id']}`",
        "",
        f"- Mode: `{report['mode']}`",
