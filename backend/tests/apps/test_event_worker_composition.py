...
def test_revocation_composition_claims_only_artifact_revocation_outbox(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, frozenset[str] | None] = {}
    ...
    monkeypatch.setattr(composition, "SqlAlchemyOutboxStore", capture_store)
    dispatcher = build_artifact_download_revocation_dispatcher(
        AppSettings(outbox_batch_size=7, outbox_max_attempts=4),
        session_factory=async_sessionmaker(class_=AsyncSession),
        redis=cast(AsyncRedisClient, object()),
    )

    assert isinstance(dispatcher, ArtifactDownloadRevocationDispatcher)
    assert captured["event_types"] == frozenset({ARTIFACT_DOWNLOADS_REVOKED_EVENT})
