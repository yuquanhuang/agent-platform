"""QuotaPolicy route contract and strong ETag behavior tests."""
...
class QuotaPolicyRouteStub:
    async def resolve_tenant_access(
        ...
        return TenantAccess(
            ...
            permissions=frozenset({"quota_policy:create"}),
        )
...
def build_app():
    ...
    stub = QuotaPolicyRouteStub()
    service = QuotaPolicyManagementService(
        cast(TenantAccessResolver, stub),
        cast(QuotaPolicyStore, stub),
        RunCapacityPolicy(max_nonterminal_runs_per_tenant=20),
    )
    return create_app(
        settings, identity_provider=provider, quota_policy_service=service
    )


@pytest.mark.asyncio
async def test_create_quota_policy_matches_frozen_shape_and_etag() -> None:
    ...
    async with httpx.AsyncClient(
        ...
        response = await client.post(
            ...
            headers={
                ...
                "Idempotency-Key": "quota-create-1",
                "X-Request-ID": "req-quota",
            },
            json={
                "name": "Default Run limits",
                "limits": {"max_nonterminal_runs_per_tenant": 10},
            },
        )

    assert response.status_code == 201
    assert response.headers["ETag"] == '"rv:1"'
    assert response.headers["X-Request-ID"] == "req-quota"
    assert response.json()["current_version"]["version_no"] == 1


def test_app_openapi_registers_all_quota_policy_operations() -> None:
